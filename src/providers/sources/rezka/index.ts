...
import { clearTrash, extractTitleAndYear, parseSubtitleLinks, parseVideoLinks } from './utils';
...
async function getMovieReleaseDatesAndUrls({
  ...
  try {
    const searchQ = `${title} ${releaseYear}`;
    const searchData = await ctx.proxiedFetcher<string>(`/engine/ajax/search.php?q=${encodeURIComponent(searchQ)}`, {
      baseUrl: rezkaBase,
    });
